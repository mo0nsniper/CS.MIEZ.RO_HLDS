...
public plugin_init()
{
	register_plugin("Admin Votes", AMXX_VERSION_STR, "AMXX Dev Team & mo0n_sniper")
...
public actionResult(id, key)
{
	...
	switch (key)
	{
		...
		case 1: {
			if(contain(g_Execute, "changelevel") != -1) 
			{
				new splited[64]
				split(g_Execute, "", 0, splited, sizeof(splited), " ")
				set_cvar_string("amx_nextmap", splited)
			}
			autoRefuse()
		}
