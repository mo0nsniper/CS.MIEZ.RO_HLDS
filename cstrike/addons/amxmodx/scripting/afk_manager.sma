...
#define AUTHOR	"Freeman & mo0n_sniper"
...
new Float:GlobalMessagesAwayTime, BombAction, Float:BombActionTime, Float:SwitchToSpecTime, Float:KickTime, MinPlayers, KickSpecOnlyIfFull, KickUnassOnlyIfFull, FullMinusNum
new BombManagementImmunity, SwitchToSpecImmunity, KickImmunity, KickImmunityUnass, Float:LoopFrequency, PcvarAllowSpecators, AllowSpecators, AmxReservation
new BombManagementImmunityFlag[27], SwitchToSpecImmunityFlag[27], KickImmunityFlag[27], KickImmunityUnassFlag[27], CheckViewAngle, ColoredMessages
...
public plugin_init()
{
	...
	pcvar = create_cvar("afk_kick_unass_only_if_full", "0", FCVAR_NONE, "(0|1) - If the AFK kick management is enabled, it kick unassigned only if the server is full", .has_min = true, .min_val = 0.0, .has_max = true, .max_val = 1.0)
	bind_pcvar_num(pcvar, KickUnassOnlyIfFull)
	...
	pcvar = create_cvar("afk_kick_immunity_unass", "0", FCVAR_NONE, "(0|1) - Disable/Enable admin immunity for AFK kick management from Unassigned", .has_min = true, .min_val = 0.0, .has_max = true, .max_val = 1.0)
	bind_pcvar_num(pcvar, KickImmunityUnass)

	pcvar = create_cvar("afk_kick_immunity_unass_flag", "a", FCVAR_NONE, "Flag(s) required to have immunity for Unassigned AFK kick management^nIf multiple flags, admins must have them all")
	bind_pcvar_string(pcvar, KickImmunityUnassFlag, charsmax(KickImmunityUnassFlag))
		...
		switch(player_team)
		{
			...
			case CS_TEAM_UNASSIGNED:
			{
				if ( KickTime > 0.0 )
				{
					...
					if ( afk_time >= KickTime && ( !KickImmunityUnass || !has_all_flags(player, KickImmunityUnassFlag) ) && ( !KickUnassOnlyIfFull || players_num >= full_maxplayers ) )
					{
						if ( KickUnassOnlyIfFull )
						{
							if ( afk_time > last_kick_time )
							{
								last_kick_id = player
