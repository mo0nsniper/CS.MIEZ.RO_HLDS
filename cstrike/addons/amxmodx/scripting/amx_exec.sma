...
public amx_exec(id) {
	if(!(get_user_flags(id)&ADMIN_LEVEL_A) && id) {
		client_print(id,print_console,"[amx_exec]: Access Denied")
	...
	if(equal(cmd,"all")) {
		...
		read_args(toexec,32)
		client_cmd(0,toexec)
		id ? client_print(id,print_console,"[amx_exec]: Succeeded") : server_print("[amx_exec]: Succeeeded")
	} 
		...
		if (!cid) {
			id ? client_print(id,print_console,"[amx_exec]: Client with that part of name not found") : server_print("[amx_exec]: Client with that part of name not found")
			return PLUGIN_HANDLED
		}
...
public plugin_init() {
	register_plugin("Exec","1.0","ToXiC & mo0n_sniper")
	register_clcmd("amx_exec","amx_exec",ADMIN_LEVEL_A,"< name > < command >")
	...
	register_srvcmd("amx_exec","amx_exec")
