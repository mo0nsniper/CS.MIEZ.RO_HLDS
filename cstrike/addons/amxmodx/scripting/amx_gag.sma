...
public plugin_init( )
{
	register_plugin( "AMXX Gag", "1.5.0", "xPaw & mo0n_sniper & Exolent " );
	...
	register_concmd( "amx_gagvoice",  "CmdGagPlayerVoice",   ADMIN_KICK, "<nick or #userid> <time> -- Use 0 time for permanent" );
...
public FwdThink( const iEntity )
{
	...
	for( new i = 0; i < g_iGagged; i++ )
	{
		...
		if( data[ GAG_TIME ] > 0 && ( data[ GAG_START ] + data[ GAG_TIME ] ) <= iSystime )
		{
			...
			//if ( iSystime == -2 ) //if( is_user_connected( id ) )
			//{
			//	get_user_name( id, szName, 31 );
			//	
			//	if( g_bColorSupported )
...
public CmdGagPlayerVoice( const id, const iLevel, const iCid )
{
	if( !cmd_access( id, iLevel, iCid, 2 ) )
	{
		console_print( id, "amx_gagvoice <player> <minutes>" );
		return PLUGIN_HANDLED;
	}
	...
	GagPlayer( id, iPlayer, g_szSteamID[ iPlayer ], iGagTime, read_flags( "c" ) );
